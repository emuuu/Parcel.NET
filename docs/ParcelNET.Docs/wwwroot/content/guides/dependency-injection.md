...
category: Guides
order: 2
description: Register and use ParcelNET services with the .NET DI container.
apiRef: DhlBuilder
---

## Overview

ParcelNET is designed for dependency injection. All services are registered via a fluent builder pattern and resolved through standard `IServiceCollection` / `IServiceProvider`.
